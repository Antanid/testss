    ...
    if ( manager ) {
      toast(
        <div className='manager-toast'>
          ❗️Это индивидуальное направление, необходимо связаться с менеджером для уточнения условий обмена по выбранному направлению - <b>@wozaexchange</b>
        </div>
        , {
      ...
      <div className='payment-modal__content'>
        <div className="payment-modal__container payment-modal__main">
          <h3 className='text text_h3'>
            {isDeposit ?
              language === 'en' ? 'Deposit balance' : 'Пополнение баланса'
              :
              language === 'en' ? 'Withdraw balance' : 'Вывод баланса'
            }
          </h3>
          ...
          <div className='payment-modal__details'>
            ...
            <div className='payment-modal__details__item'>
              <h4 className='text text_h4 text_h4_light'> 
                {isDeposit ?
                  language === 'en' ? 'Select a deposit method' : 'Выберите способ пополнения'
                  :
                  language === 'en' ? 'Select a withdrawal method' : 'Выберите способ вывода'
                }
              </h4>
              <div
                className='payment-modal__details__item__input-container select-container'
              >
                <div className='icon__container' onClick={() => setMethodToggle(prev => !prev)} >
                  <svg className={`icon__svg ${methodShow && 'icon__svg_active'}`} viewBox="0 0 11 19" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <input
                  ...
                  type="text"
                  value={paymentMethods[operation].find(method => method.id === isPaymentMethod)?.[language]}
                  readOnly
                  onClick={() => setMethodToggle(prev => !prev)}
                />
                
                {/* ===========| Окно выбора |=========== */}
                <div className={`payment-modal__container payment-modal__method ${methodShow ? 'payment-modal__method_show' : ''}`}>
                  <div className='payment-modal__method__title'>
                    <h4 className='text text_h4 select-method__title'>
                      {isDeposit ?
                        language === 'en' ? 'Select a deposit method' : 'Выбрать метод пополнения'
                        :
                        language === 'en' ? 'Select a withdrawal method' : 'Выбрать метод вывода'
                      }
                      <button className='select-method__close button_close' onClick={() => setMethodToggle(false)}>✕</button>
                    </h4>
                  </div>
                    ...
                    <div className='input-container select-method__input-container'>
                      <input
                        ...
                        placeholder={isDeposit ?
                          language === 'en' ? 'Deposit method' : 'Метод пополнения'
                          :
                          language === 'en' ? 'Withdraw method' : 'Метод вывода'
                        }
                      />

                      <div className='select-method__icon-container'>
                        <svg className="icon select-method__icon" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
                          <path d="M14.3333 14.3333L21 21M8.77778 16.5556C4.48223 16.5556 1 13.0733 1 8.77778C1 4.48223 4.48223 1 8.77778 1C13.0733 1 16.5556 4.48223 16.5556 8.77778C16.5556 13.0733 13.0733 16.5556 8.77778 16.5556Z"
                          strokeWidth="2" />
                        </svg>
                      </div>
                    </div>
                            ...
                            >
                              <h4 className='text text_h4 text_h4_light'>
                                ...
                                <span className='select-point'>
                                  <svg className='icon' viewBox="0 0 15 10" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M1.66797 5.00002L5.66825 9L13.668 1" strokeWidth="2" />
                                  </svg>
                                </span>
                              </h4>
                            </button>
            ...
            <div className='payment-modal__details__item'>
              <h4 className='text text_h4 text_h4_light'> 
                {isDeposit ? (
                  language === 'en' ?
                    <>
                      Amount to deposit {error && <span className="input-error-label">{error}</span>}
                    </>
                    :
                    <>
            ...
            <button className='button button_accent button_submit' onClick={handleSendOperation}>
              {isDeposit ?
                ...
                :
                language === 'en' ? 'Start withdrawing funds' : 'Начать вывод средств'
              }
            </button>
            {/* ============================================== */}
          </div>
