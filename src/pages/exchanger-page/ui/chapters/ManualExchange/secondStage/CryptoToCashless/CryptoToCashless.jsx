...
export const CryptoToCashless = () => {
  ...
  // Стейт выбранного метода пополнения
  ...
  const handlePaymentMethodClick = option => {
    const fullMethod = paymentMethods.find(m => m.id === option.id)
    setSelectedPaymentMethod(fullMethod)
  }
  ...
  const paymentMethodOptions = paymentMethods.map(method => ({
    id: method.id,
    title: language === 'en' ? method.en : method.ru,
  }))
        ...
        <div style={{ marginTop: '1rem' }}>
          ...
          <div style={{ marginTop: '1rem' }}>
            <AutocompleteSelect
              ...
              headerTitle={
                language === 'en' ? 'Select a payment method' : 'Выбрать метод пополнения'
              }
...
const paymentMethods = [
  { id: 1, ru: 'Карта', en: 'Card' },
  { id: 2, ru: 'IBAN', en: 'IBAN' },
  { id: 3, ru: 'Другой счет (уточню у менеджера)', en: 'Other account (will consult manager)' },
]
