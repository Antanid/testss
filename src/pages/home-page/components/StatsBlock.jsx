...
export const StatsBlock = ({ language, Icons, Images }) => {
  ...
  return (
    <section className={`${styles.b1} ${styles.stats}`}>
      <BlockTitle
        className={`block_title ${TitleVisible ? 'visible_left' : ''}`}
        language={language}
        Icons={Icons}
        ref={refTitle}
        en="Statistical factors of influence"
        ru="Статистические факторы влияния"
      />
      <div
        className={`${styles.stats_block_cont} ${ContainerVisible ? 'visible_top' : ''}`}
        ref={refContainer}
      >
        ...
        {statsContent.map((item, index) => {
          ...
          return (
            ...
            >
              ...
              <h4>
                <i>{item.description[language] || item.description.ru}</i>
              </h4>
            </div>
          )
        })}
